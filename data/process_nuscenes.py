...
if __name__ == "__main__":
    ...
    map_version = '0.1'
    splits = ['mini_train', 'mini_val', 'mini_test']
    ...
    for split in splits:
        ...
        for scene_name in split_scenes:
            ...
            scene_data_orig = prediction_scenes.get(scene_name, []) # What does prediction_scenes.get returns, look from nuscenes -mg
            ...
            while True:
                if sample['token'] in all_samples:
                    ...
                    for ann_token in sample['anns']:
                        ...
                        data[10] = annotation['size'][0] #X, annotations given as global coordinate system. -mg
                        data[11] = annotation['size'][2] #Y, y is height in our case and ignored in prredictions. -mg
                        data[12] = annotation['size'][1] #Z
                        data[13] = annotation['translation'][0]
                        data[14] = annotation['translation'][2]
                        data[15] = annotation['translation'][1]
                        data[16] = Quaternion(annotation['rotation']).yaw_pitch_roll[0] #counter clock-wise rotation around z-axis.  
...
"""
this code takes nuScenes dataset and splits it into train,val and test, it also converts all data into annotations and feed them into model.
it also generates maps for visualisation. this code uses nuScenes library, this library might be a problem for different dataset process.(level5 has its own process.py)  
"""

"""
code lines 76-156 has to change what can we use for #def get_prediction_challenge_split ? ,
data needs to be divided into tain,test,val and annotations must be created, what are the tokens ? (they came from prediction_challange.py)
"""
"""
lines between 158 and 210 is not needed they create maps for visiualization.
"""
"""
first creates an empty array.zeros(18), than multiplt it with -1.0 (why ?) then adds 
frame_id,instances,size,translation,rotation.(it uses nuscenes.get for annotations)  
"""
"""
translation,rotation,size comes from ANNOTATİON, 
"""