    ...
    def vis_all(self, output, wait_time=1):
        ...
        if (output != {}):
            ...
            if self.config['use_radar_polar']:
                ...
                cv2.imshow('radar', output['sensors']['radar_cartesian'])
                if self.config['save_images']:
                    cv2.imwrite(os.path.join(self.output_folder,  str(
                        self.current_time), 'radar_polar.png'), output['sensors']['radar_cartesian'])
            ...
            if self.config['use_proj_lidar_left']:
                ...
                cv2.imshow('projected lidar to left camera', overlay_left) #original was overlay_left_bb
                if self.config['save_images']:
                    cv2.imwrite(os.path.join(self.output_folder,  str(
                        self.current_time), 'overlay_left_bb.png'), overlay_left)
            if self.config['use_proj_lidar_right']:
                ...
                cv2.imshow('projected lidar to right camera', overlay_right) #original was overlay_right_bb
                if self.config['save_images']:
                    cv2.imwrite(os.path.join(self.output_folder, str(
                        self.current_time), 'overlay_right_bb.png'), overlay_right)
