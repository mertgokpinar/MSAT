    ...
    for i in range(len(valid_id)):    # number of agents
        ...
        for j in range(cfg.future_frames):
            ...
            if len(cur_data) > 0 and identity in cur_data[:, 1]:
                ...
                data[0] = frame + j + 1
            # print('original_data',data[[10,11]]) #debug code
            data[[13, 15]] = pred[i, j].cpu().numpy() 
            # print('pred_data',pred[i,j]) #debug code
            most_recent_data = data.copy()
            pred_arr.append(data)
        pred_num += 1
#TODO make sure you corrected indices values witth original code -mg
    if len(pred_arr) > 0:
        pred_arr = np.vstack(pred_arr)
        indices = [0, 1, 13, 15]       # frame, ID, x, z (remove y which is the height). Originally nuscenes drops y since it represents height in nuscenes 3Dbbox. -mg
        pred_arr = pred_arr[:, indices]
        # print('pred_arr',pred_arr) #debug
        # save results
        fname = f'{save_dir}/{seq_name}/frame_{int(frame):06d}{suffix}.txt'
        # print('fname',fname)#debug
        mkdir_if_missing(fname)
        np.savetxt(fname, pred_arr, fmt="%.3f") #saves ground data t0
        # breakpoint() #debug
    return pred_num
...
def test_model(generator, save_dir, cfg):
    ...
    while not generator.is_epoch_end():
        ...
        gt_motion_3D = torch.stack(data['fut_motion_3D'], dim=0).to(device) * cfg.traj_scale #gets ground trurh data from dataset -mg
        with torch.no_grad():
            recon_motion_3D, sample_motion_3D = get_model_prediction(data, cfg.sample_k) #gets predicted data from model -mg
        ...
        save_prediction(recon_motion_3D, data, '', recon_dir)        # save recon, saves prediction to text file. -mg
        num_pred = save_prediction(gt_motion_3D, data, '', gt_dir)              # save gt, saves ground trurh data to txt file -mg
...
if __name__ == '__main__':
    ...
    for epoch in epochs:
        ...
        """ model """
        if not args.cached:
            model_id = cfg.get('model_id', 'msat')
