...
class msat_single(nn.Module):
    def __init__(self, cfg):
        ...
        if self.use_map:
            ...
            #self.map_encoder = MapEncoder(cfg.map_encoder,ful_con=6688 if self.sensor=='camera' else 30752)
            #self.map_encoder = MapEncoder(cfg.map_encoder,ful_con=32 if self.sensor=='camera' else 196)
            self.map_encoder = MapEncoder(cfg.map_encoder,ful_con=253 if self.sensor=='camera' else 196)
            self.ctx['map_enc_dim'] = self.map_encoder.out_dim

        else: print('SENSOR:',self.sensor)
    ...
    def set_data(self, data):
        ...
        self.data['seq'] = in_data['seq']
        self.data['frame'] = in_data['frame']
        ...
        if self.use_map:
            ...
            self.data['agent_sensors']=data['sensors'].expand(self.data['batch_size'],-1,-1,-1).to(device)#expanding sensor tensor with batch size and rellocating in memory -mg

            #print ('sensor_tensor',data['sensors'].element_size() * data['sensors'].nelement())
    ...
    def forward(self):
        ...
        if self.use_map:
            self.data['map_enc']= self.map_encoder(self.data['agent_sensors']) # sends sensor data to mapencoder -mg  
            #self.data['map_enc']=torch.rand([self.data['batch_size'],32], device = 'cuda') # EXPERIMENTAL REMOVE LATER -mg
            #EXPERIMENTAL REMOVE LATER -mg
    ...
    def compute_loss(self):
        ...
        return total_loss, loss_unweighted_dict,transformer_loss
