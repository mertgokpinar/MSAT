...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='radiate')
    ...
    for seq_name in seq_eval:
        ...
        for data_file in data_filelist:      # each example e.g., seq_0001 - frame_000009
            ...
            metrics = pd.DataFrame({'Sequence':[seq_name],'Frame':[int(frame_list[0])],'ADE':[stats_meter['ADE'].val],'FDE':[stats_meter['FDE'].val],'Weather':[wheather], 'Number of Agents':len(id_list)})
            log = log.append(metrics, ignore_index = True)
            log.to_csv(os.path.join(results_dir,'results_detailed_log.csv'))       
    ...
    print('Results are saved to: ',results_dir)
    log_file.close()
